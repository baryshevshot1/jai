...
export const state = {
  // Модель, на которой ПОЙДЁТ обычный текстовый вопрос. Не выбирается в шапке:
  // приложение подбирает её само (роль + железо), см. models.ts → applyModelChoice.
  // Пустая строка = отвечать нечем (нет моделей / нет движка) — композер выключен.
  selectedModel: "",
  // Ручное переопределение из настроек: "auto" (по умолчанию) либо тег модели.
  // Хранится в settings.json (ключ model_choice).
  modelChoice: "auto",
  // Уровень железа из «светофора»: "green" | "yellow" | "red" | "" (ещё не измерено).
  // От него зависит, какую текстовую модель берёт авто-выбор (9b против 4b).
  hwTier: "",
  ...
  thinkEnabled: true,
  // Щадящий режим: нейросеть использует половину ядер CPU (движок и так уступает
  // системе по приоритету) — слабые машины меньше греются. Пока пользователь не
  // выбрал сам (gentleAuto), значение выставляется по «светофору» железа.
  gentleMode: false,
  gentleAuto: true,
