...
export function setStreaming(on: boolean) {
  ...
  // Ответ закончился — включаем композер ТОЛЬКО если есть чем отвечать.
  //
  // Прежде здесь стояло безусловное `inputEl.disabled = on`, и это теряло отказ,
  // случившийся во время ответа: подбор модели намеренно не трогает композер, пока
  // идёт стрим (иначе поле выключилось бы на полуслове), а после стрима состояние
  // никто не пересчитывал. Достаточно было движку умереть посреди длинного ответа
  // — список моделей становился пуст, а поле ввода по завершении включалось как ни
  // в чём не бывало. Человек печатал вопрос, жал Enter, и не происходило НИЧЕГО:
  // ни ответа, ни ошибки.
  setComposerEnabled(!!state.selectedModel);
  if (state.selectedModel) inputEl.focus(); // вернуть фокус в поле после ответа
}

// Переключатель-тумблер: вид И состояние для экранного диктора одной операцией.
//
// Прежде состояние выражалось ТОЛЬКО CSS-классом. Незрячий пользователь слышал
// «Онлайн-режим, кнопка» и при включённом, и при выключенном режиме — то есть не
// мог установить, уходят ли данные его запроса на внешний сервер. Для продукта,
// который обещает 152-ФЗ, это не мелочь доступности, а невозможность проверить
// главное обещание. У кнопки микрофона рядом aria-pressed стоял с самого начала —
// расхождение внутри одного ряда.
export function setToggleState(el: HTMLElement, on: boolean) {
