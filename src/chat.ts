...
// Насколько далеко назад картинка ещё считается «относящейся к разговору». Без этой
// отсечки диалог, где картинку присылали хоть раз, НАВСЕГДА оставался на модели
// зрения: каждый следующий текстовый вопрос уходил ей, а base64 старых снимков
// (мегабайты) прикладывался к каждому запросу заново. Четыре сообщения — это вопрос
// с картинкой, ответ и уточняющий вопрос: ровно тот случай, ради которого картинку
// и держат в контексте.
const IMAGE_RECENCY_WINDOW = 4;

// Есть ли в диалоге картинки. От этого зависит, по карману ли частые автосохранения:
// изображения хранятся base64-строками, и файл диалога переписывается целиком.
function historyHasImages(): boolean {
  return history.some((m) => m.images?.length);
}

// Индексы ходов истории, у которых сохраняем картинки: последние IMAGE_HISTORY_TURNS
// ходов с изображениями, но только среди свежих сообщений. По этому же множеству
// generate() решает, нужна ли модель зрения, — до сборки сообщений (ей уже нужен план).
function recentImageTurns(): Set<number> {
  const keep = new Set<number>();
  const oldest = Math.max(0, history.length - IMAGE_RECENCY_WINDOW);
  for (let i = history.length - 1, kept = 0; i >= oldest && kept < IMAGE_HISTORY_TURNS; i--) {
...
function startTurn(myGen: number) {
  ...
  // Живая печать с форматированием: renderMarkdownInto каждый раз разбирает ВЕСЬ
  // накопленный ответ заново (markdown-it не умеет дорисовывать только хвост), поэтому
  // на ФИКСИРОВАННОМ интервале суммарная работа за стрим растёт квадратично от длины
  // ответа — число перерисовок пропорционально длине, и каждая стоит O(длины). На
  // ответе в 8–10 тыс. символов это сотни полных перерендеров одного и того же текста,
  // заметные фризы и дёрганая прокрутка. Вместо фиксированного интервала растягиваем
  // паузу вместе с уже напечатанным текстом: на короткий ответ она как раньше (~180мс,
  // глаз не замечает), а на длинном — растёт, и перерисовки происходят на всё более
  // редких, но всё более длинных «скачках». Это даёт геометрическую, а не арифметическую
  // последовательность точек перерисовки — суммарная работа за весь стрим приближается
  // к линейной от длины ответа, а не к квадратичной. Финальный рендер по завершении
  // (finalize) от этой логики не зависит и остаётся авторитетным — рисуется начисто.
  const LIVE_RENDER_MS_BASE = 180; // короткий ответ — как раньше, без задержек
  const LIVE_RENDER_MS_GROWTH = 0.15; // мс добавки на символ уже накопленного ответа
  const LIVE_RENDER_MS_MAX = 1500; // очень длинный ответ — не реже раза в 1.5 с
  const liveRenderInterval = () =>
    Math.min(LIVE_RENDER_MS_MAX, LIVE_RENDER_MS_BASE + answer.length * LIVE_RENDER_MS_GROWTH);
  ...
  onEvent.onmessage = (msg) => {
    ...
    if (msg.type === "status") {
      ...
      if (answer) {
        ...
        freezeReason(true);
        // Живая печать с Markdown, но не чаще liveRenderInterval(): свежий кусок либо
        // рисуем сразу (пауза прошла), либо ставим отложенную перерисовку. Интервал
        // считаем заново на каждый чанк — он растёт вместе с ответом (см. выше).
        ...
        //
        // В диалогах с картинками автосейв пропускаем. Изображения лежат в истории
        // строками base64, а сохранение переписывает файл ЦЕЛИКОМ: каждые несколько
        // секунд пришлось бы заново сериализовать десятки мегабайт, прогнать их
        // через IPC и сбросить на диск — параллельно работе модели, которой память
        // нужнее. Выбрасывать картинки из черновика нельзя: после краха файл остался
        // бы без них. Цена отказа — незавершённый ответ, и только в таких диалогах.
        if (Date.now() - lastDraftTs >= DRAFT_SAVE_MS && !historyHasImages()) {
          lastDraftTs = Date.now();
          persist({ role: "assistant", content: answer });
        }
      }
      // scrollToBottom() здесь раньше звался на КАЖДЫЙ чанк (то есть на каждый токен) —
      // отдельно от вызова внутри paintLive(). Между перерисовками лента не меняется
      // (ui.msg трогает только paintLive), так что промежуточные вызовы читали и писали
      // scrollTop впустую — лишний layout-реflow на каждый токен, дёрганая прокрутка.
      // Скролл теперь идёт только вместе с фактической перерисовкой, той же кадрой.
    }
