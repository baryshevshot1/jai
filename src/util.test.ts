...
// Размеры меньше гигабайта: gb() показывал модель в 465 МБ как «0.5 ГБ», а остаток
// в 40 МБ — как «0.0 ГБ», то есть «осталось ноль». Человек на это либо решает, что
// всё уже скачано, либо жмёт «Начать заново» и выбрасывает полчаса загрузки.
describe("size: размер в той единице, в которой он читается", () => {
  it("модель распознавания речи не превращается в ноль гигабайт", async () => {
    const { size } = await import("./util");
    const model = 487_601_967; // ggml-small.bin, ровно
    expect(size(model)).toBe("465 МБ");
    // Остаток, на котором прежняя формула писала «0.0 ГБ».
    expect(size(40 * 1024 * 1024)).toBe("40 МБ");
    expect(size(1024 * 1024)).toBe("1.0 МБ");
  });

  it("гигабайты остаются гигабайтами, мелочь — килобайтами", async () => {
    const { size, gb } = await import("./util");
    const big = 5.4 * 1024 * 1024 * 1024;
    expect(size(big)).toBe(gb(big)); // модели движка меряются как раньше
    expect(size(2048)).toBe("2 КБ");
    expect(size(0)).toBe("0 КБ");
  });
});
