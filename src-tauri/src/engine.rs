...
fn spawn(exe: &Path, models_dir: Option<&Path>) -> std::io::Result<Child> {
    ...
    // - MAX_LOADED_MODELS=2: чат-модель И модель поиска по документам. Раньше стояла
    //   единица — ради защиты от свопа, но она давала обратный эффект: поиск перед
    //   каждым ответом загружал bge-m3 и ВЫТЕСНЯЛ чат-модель, после чего её весы
    //   (6+ ГБ) читались с диска заново — десятки секунд на каждый вопрос. Теперь
    //   помещаются обе, и это не стоит видеопамяти: модель поиска считается на
    //   процессоре (num_gpu: 0 в embed.rs), а зрение всё так же вытеснит текстовую.
    // - NUM_PARALLEL=1: не умножать память на параллельные запросы.
    // - FLASH_ATTENTION + KV_CACHE_TYPE=q8_0: KV-кэш примерно вдвое меньше.
    // - KEEP_ALIVE: умеренный; в самих запросах он задаётся точнее (chat.rs, embed.rs),
    //   и только так он действует на системный движок, которому наши env не достаются.
    cmd.env("OLLAMA_MAX_LOADED_MODELS", "2");
